...
-- admin_weights (public read for UI)
...
-- Idempotent cron: unschedule if exists, then schedule
select cron.unschedule('ingest-github-daily-0400');
select
  cron.schedule(
...
select cron.unschedule('rank-recompute-daily-0530');
select
  cron.schedule(
